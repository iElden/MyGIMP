...
namespace Mimp {
    ...
    Layer &LayerManager::selectLayer(unsigned int layer) {
        if (layer >= this->_layers.size())
            throw InvalidArgumentException(
                    "Cannot select layer " + std::to_string(layer) + (
                            this->_layers.size() == 1 ?
                            " because there is only 1 layer." :
                            " because there are only " +
                            std::to_string(this->_layers.size()) + " layers.")
            );
    ...
    void LayerManager::deleteLayer(unsigned int layer) {
        if (layer >= this->_layers.size())
            throw OutOfBoundException("Cannot delete layer " + std::to_string(layer) + (
                    this->_layers.size() == 1 ?
                    " because there is only 1 layer." :
                    " because there are only " +
                    std::to_string(this->_layers.size()) + " layers."));
        if (this->_layers.size() == 1)
            throw InvalidArgumentException("Cannot delete layer because there is only a single layer.");
        this->_setBusy();
    ...
    void LayerManager::loadMimpImage(const std::string &path) {
        ...
        try {
            ...
            for (unsigned i = 0; i < image->nbLayers; i++) {
                ...
                if (reinterpret_cast<uintptr_t>(&layer->pixels[layer->size.x * layer->size.y]) >
                    reinterpret_cast<uintptr_t>(&buffer[length]))
                    throw InvalidImageException(
                            "Unexpected end of file when parsing layer " + std::to_string(layers.size()));
    ...
    void LayerManager::save(std::ostream &stream) const {
        ...
        for (auto &layerObject : this->_layers) {
            ...
            std::memcpy(layer->pixels, layerObject->buffer.getBuffer(),
                        layer->size.x * layer->size.y * sizeof(*layer->pixels));
