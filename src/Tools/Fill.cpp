...
{
	if (layer.getFrameBuffer().posIsOutOfBound(pos))
		return;
	auto target_color = layer.getFrameBuffer().getPixel(pos);
...
void Mimp::Fill::_spread_color(Vector2<int> pos, Layer &layer, Color target_color, Color fill_color)
{
	...
	layer.getFrameBuffer().drawPixel(pos, fill_color);
	while (!points.empty()) {
		new_points.clear();
		for (auto pt : points)
			for (auto sp : spread_pos) {
				auto new_pos = pt + sp;
				if (!layer.getFrameBuffer().posIsOutOfBound(new_pos) && layer.getFrameBuffer().getPixel(new_pos).diff(target_color, this->_alpha_in_tolerance) <= this->_tolerance) {
					layer.getFrameBuffer().drawPixel(new_pos, fill_color, SET);
