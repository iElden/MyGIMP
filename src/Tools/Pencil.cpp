...
namespace Mimp
{
	...
	void Pencil::onMouseDrag(Vector2<int> oldPos, Vector2<int> newPos, MouseClick click, Image &image)
	{
		...
		layer.getFrameBuffer().drawLine(oldPos - layer.pos, newPos - layer.pos, this->_box.getSelectedColor(click), this->_radius, this->_shape);
	}
	...
	void Pencil::onClick(Vector2<int> pos, MouseClick click, Image &image)
	{
		...
		layer.getFrameBuffer().drawAt(pos - layer.pos, this->_box.getSelectedColor(click), this->_radius, this->_shape);
	}
